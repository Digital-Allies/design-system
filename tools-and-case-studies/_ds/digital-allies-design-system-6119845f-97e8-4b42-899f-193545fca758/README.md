...
This system is the canonical source for all Digital Allies brand work. It's
hosted on GitHub Pages at https://brand.digitalallies.net/ and sourced from:

- **GitHub repos:**
  - https://github.com/Digital-Allies/DigitalAllies — Primary live site (HTML/CSS/JS, the source of truth for visual implementation). Contains `index.html`, `brand.html`, department icons, `suspension bridge.svg`, `favicon.svg`, and architecture diagrams.
  - https://github.com/Digital-Allies/design-system — This design system (you are here). Contains all brand tokens, components, templates, and assets.

- **Design working files** (reference only):
  - Style guide, voice guides, logo system, icon system, brand art, photography, and social templates are maintained within this repo's `assets/` folder.

The reader is encouraged to explore the live website at
https://digitalallies.net to see the brand in production context.
...
| `ui_kits/website/ContentCalendar.jsx` + `.d.ts` | The one **exposed, reusable component** (`window.DigitalAlliesDesignSystem_611984.ContentCalendar`). 30-day marketing calendar — sortable, filterable, CMS-hookable. See its card `ui_kits/website/content-calendar.html`. |
...
> reusable design-system component (it ships a `.d.ts` and a bundle-loading
> card). The other `ui_kits/website/*.jsx` (Nav, Hero, Departments, …) and
> `social/*.jsx` are **page partials** — composition pieces of their own
> demo pages, shared via `window` for Babel-standalone, not meant to be
